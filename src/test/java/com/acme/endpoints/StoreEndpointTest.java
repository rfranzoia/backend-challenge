...
public class StoreEndpointTest {
	...
	public void testListStores() throws InterruptedException {
		...
		ResponseEntity<StoreDTO[]> response = this.restTemplate.exchange(builder.build().encode().toUri(),
				HttpMethod.GET, entity, StoreDTO[].class);
	...
	public void testListCarsAuthenticationFail() throws InterruptedException {
		...
		ResponseEntity<StoreDTO[]> response = this.restTemplate.getForEntity(builder.build().encode().toUri(),
				StoreDTO[].class);
